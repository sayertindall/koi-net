...
from koi_net import EdgeModel, EventArrayModel, EventQueueModel, KoiNetPath, NodeModel, NodeType
...
class NetworkAdapter:
    ...
    def handshake(self, node: RID, **kwargs):
        node_profile = self.state.get_node(node)
        
        httpx.post(
            url=node_profile.base_url + KoiNetPath.STATE_RIDS,
            data=HandshakeModel(**kwargs).model_dump_json()
        )
    
    def broadcast_events(self, node: RID, **kwargs):
        node_profile = self.state.get_node(node)
                
        httpx.post(
            url=node_profile.base_url + KoiNetPath.STATE_RIDS,
            data=EventArrayModel(**kwargs).model_dump_json()
        )
